...
class Uebebiene_Teacher_App_Routes {
  private const QUERY_FLAG = 'uebebiene_teacher_app';
  private const QUERY_ASSET = 'uebebiene_teacher_asset';

  public function register_rewrite_rules(): void {
    add_rewrite_rule('^' . Uebebiene_Teacher_App::APP_SLUG . '/?$', 'index.php?' . self::QUERY_FLAG . '=1', 'top');
    add_rewrite_rule('^' . Uebebiene_Teacher_App::APP_SLUG . '/(.+)$', 'index.php?' . self::QUERY_FLAG . '=1&' . self::QUERY_ASSET . '=$matches[1]', 'top');
  }
  ...
  public function get_plugin_settings(): array {
    $settings = get_option(Uebebiene_Teacher_App::SETTINGS_OPTION, []);
  ...
  public function get_learner_app_url(): string {
    $settings = $this->get_plugin_settings();
    ...
    $bridge_settings = get_option('uebebiene_sync_bridge_settings', []);
    $bridge_configured = is_array($bridge_settings) ? trim((string) ($bridge_settings['learner_app_url'] ?? '')) : '';
    if ($bridge_configured !== '') {
      return untrailingslashit($bridge_configured) . '/';
    }

    return 'https://marsrakete.github.io/uebebiene/';
  }

  public function get_learner_app_url_source(): string {
    $settings = $this->get_plugin_settings();
    if (trim((string) ($settings['learner_app_url'] ?? '')) !== '') {
      return 'Teacher-App-Plugin';
    }

    $bridge_settings = get_option('uebebiene_sync_bridge_settings', []);
    if (is_array($bridge_settings) && trim((string) ($bridge_settings['learner_app_url'] ?? '')) !== '') {
      return 'Sync Bridge';
    }

    return 'Fallback auf GitHub Pages';
  }
  ...
  public function get_runtime_config(): array {
    return [
      ...
      'teacherAppUrl' => $this->get_app_url(),
      'shareUrl' => $this->get_learner_app_url(),
      ...
      'qrShareUrl' => $this->get_learner_app_url(),
  ...
  private function serve_shell(): void {
    ...
    require UEBEBIENE_TEACHER_APP_PATH . 'templates/teacher-app-shell.php';
    exit;
  }
  ...
  private function serve_manifest(): void {
    ...
    $manifest = [
      'id' => wp_parse_url($app_url, PHP_URL_PATH) ?: '/' . Uebebiene_Teacher_App::APP_SLUG . '/',
      'name' => 'ÜbeBiene Lehrkräfte',
      'short_name' => 'Lehrkräfte',
      'description' => 'Lehrkräfte-PWA für ÜbeBiene mit Klassenübersicht, Kopplung und Berichten.',
      'start_url' => $app_url,
      'scope' => $app_url,
      'display' => 'standalone',
  ...
  private function serve_service_worker(): void {
    ...
    $app_path = wp_parse_url($this->get_app_url(), PHP_URL_PATH) ?: '/' . Uebebiene_Teacher_App::APP_SLUG . '/';
    $version = UEBEBIENE_TEACHER_APP_VERSION;
    $assets = [
      './',
      './teacher.css',
      './teacher.js',
    ...
    echo "const CACHE_NAME = 'uebebiene-teacher-app-{$version}';\n";
...
JS;
  ...
  private function serve_static_asset(string $asset): void {
    ...
    $base = realpath(UEBEBIENE_TEACHER_APP_PATH . 'assets');
    $file = realpath(UEBEBIENE_TEACHER_APP_PATH . 'assets/' . $normalized);
    ...
    status_header(200);
    header('X-Robots-Tag: noindex, nofollow, noarchive', true);
    if (in_array($extension, ['js', 'webmanifest'], true)) {
      nocache_headers();
    } else {
